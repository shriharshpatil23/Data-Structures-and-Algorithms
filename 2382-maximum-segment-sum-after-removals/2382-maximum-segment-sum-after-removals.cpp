...
public:
    ...
    For current index (cur), find left removed index (L) and right removed index (R)
    Delete current segment sum, pre[R-1] - pre[L]
    Insert left segment sum, pre[cur-1] - pre[L]
    Insert right segment sum, pre[R-1] - pre[cur]
    Take the maximum from all sums
    */
    #define ll long long
